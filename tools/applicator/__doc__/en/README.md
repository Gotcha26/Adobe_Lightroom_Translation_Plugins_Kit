...
However, using the central launcher ***LocalizationToolKit.py*** is generally preferred because it:
...
After application (and only if dry-run mode is not active), ***Applicator***
analyses the `TranslatedStrings_xx.txt` files present at the plugin root and
proposes an action adapted to one of three cases.

### Case 1: First Installation (no file at the root)

No `TranslatedStrings_xx.txt` exists yet. ***Applicator*** offers to copy the
template generated by ***Extractor*** directly to the plugin root:

```
It seems this is your first language installation
for translation.
Would you like to use the latest file extracted
from Extractor?

  TranslatedStrings_en.txt  →  <plugin>\TranslatedStrings_en.txt

Copy the file? [Y/n]:
```

If the user answers **no**, a sub-menu appears:

```
  1  Choose a file from the extraction
  2  Import a file from outside
  0  Exit without copying
```

> Option **1** is useful when ***Extractor*** generated templates for several
> languages in a single extraction run.

### Case 2: Same Language Already Present (update)

A file with the same language as the template already exists at the root.
***Applicator*** offers to replace it with the latest version:

```
You have likely just updated your keys in your Lua files.
A TranslatedStrings_en.txt file already exists at the root
of your plugin.
It is strongly recommended to replace it in order to update
it with your latest changes.

Replace the file already present? [Y/n]:
```

### Case 3: Multiple Languages Detected → AUTOSYNC

Several `TranslatedStrings_xx.txt` files are found at the root
(e.g. `_en.txt` + `_fr.txt`). This indicates that a manual update was made
outside the toolkit. ***Applicator*** automatically launches ***AUTOSYNC***
(from `tools/translator/autosync.py`) to synchronise all languages with the
latest keys:

```
Multiple languages detected at the plugin root:
  - TranslatedStrings_en.txt
  - TranslatedStrings_fr.txt

This indicates a manual update without using the standard tools.
Automatic launch of AUTOSYNC to synchronise everything.
...
```mermaid
flowchart TD
    A["End of application"] --> B{"TranslatedStrings\nfiles at root ?"}
    B -->|No| C{"Template in\nthe extraction ?"}
    C -->|No| D["Manual\ninstructions"]
    C -->|Yes| E["CASE 1 — First\ninstallation"]
    E --> F{"Yes"}
    E --> G{"No → sub-menu"}
    B -->|Yes| H{"Multiple\nlanguages ?"}
    H -->|Yes| I["CASE 3 — AUTOSYNC"]
    H -->|No| J{"Same language\nas template ?"}
    J -->|Yes| K["CASE 2 — Update"]
    J -->|No| L["Info: nothing\nto do"]
...
|---------|------|---------------|
| 7.2 | 2026-02 | Translation management: 3 cases + sub-menu + AUTOSYNC |
...
| 📜 | Traceability |  |  |
|--|--|--|--|
| **Name** | *README.md* | **Version** | 7.2 |
| **Type** | User Guide - APPLICATOR Advanced | **Language** | EN - *[FR](../fr/Lisez-moi.md)* |
| **GitHub Project** | [Adobe Lightroom Translation Toolkit](https://github.com/Gotcha26/Adobe_Lightroom_Translation_Plugins_Kit) | **Date** | 2026-02-03 |
| **License** | Open source | | |
