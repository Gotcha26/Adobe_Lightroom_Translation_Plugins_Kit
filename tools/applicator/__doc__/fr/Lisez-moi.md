...
Après l'application (et uniquement si le mode dry-run n'est pas activé),
***Applicator*** analyse les fichiers `TranslatedStrings_xx.txt` présents à la
racine du plugin et propose une action adaptée selon trois cas.

### Cas 1 : Première installation (aucun fichier à la racine)

Aucun `TranslatedStrings_xx.txt` n'existe encore. ***Applicator*** propose de
copier le template généré par ***Extractor*** directement à la racine :

```
Il semble que ce soit votre première installation
d'une langue à traduire.
Utiliseriez-vous le dernier fichier en date
extrait depuis Extractor ?
...
Si l'utilisateur répond **non**, un sous-menu apparaît :

```
  1  Choisir un fichier depuis l'extraction
  2  Importer un fichier depuis l'extérieur
  0  Sortir sans copier
```

> L'option **1** est utile lorsque plusieurs langues ont été générées par
> ***Extractor*** en une seule extraction.

### Cas 2 : Même langue déjà présente (mise à jour)

Un fichier de la même langue que le template existe déjà à la racine.
***Applicator*** propose de le remplacer par la version la plus récente :

```
Vous venez certainement de mettre à jour vos clés dans votre lua.
Un fichier TranslatedStrings_en.txt se trouve déjà à la racine
de votre plugin.
Il est très fortement conseillé de le remplacer afin de le mettre
à jour avec votre dernière mise à jour.

Remplacer le fichier déjà présent? [O/n]:
```

### Cas 3 : Plusieurs langues détectées → AUTOSYNC

Plusieurs fichiers `TranslatedStrings_xx.txt` sont trouvés à la racine
(ex : `_en.txt` + `_fr.txt`). Cela signifie qu'une mise à jour manuelle a été
effectuée en dehors du toolkit. ***Applicator*** lance automatiquement
***AUTOSYNC*** (depuis `tools/translator/autosync.py`) pour synchroniser
l'ensemble des langues avec les dernières clés :

```
Plusieurs langues détectées à la racine du plugin :
  - TranslatedStrings_en.txt
  - TranslatedStrings_fr.txt

Cela indique une mise à jour manuelle sans passer par les outils standard.
Lancement automatique de AUTOSYNC pour synchroniser l'ensemble.
...
```mermaid
flowchart TD
    A["Fin de l'application"] --> B{"fichiers\nTranslatedStrings\n à la racine ?"}
    B -->|Non| C{"Template dans\nl'extraction ?"}
    C -->|Non| D["Instructions\nmanuelle"]
    C -->|Oui| E["CAS 1 — Première\ninstallation"]
    E --> F{"Oui"}
    E --> G{"Non → sous-menu"}
    B -->|Oui| H{"Plusieurs\nlangues ?"}
    H -->|Oui| I["CAS 3 — AUTOSYNC"]
    H -->|Non| J{"Même langue\nque le template ?"}
    J -->|Oui| K["CAS 2 — Mise\nà jour"]
    J -->|Non| L["Info : rien\nà faire"]
...
|---------|------|---------------|
| 7.2 | 2026-02 | Gestion des traductions : 3 cas + sous-menu + AUTOSYNC |
