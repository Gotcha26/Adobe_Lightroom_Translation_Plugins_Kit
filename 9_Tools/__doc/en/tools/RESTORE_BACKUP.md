...
|------|-------------|-----------------|
| **Restoration** | Actual restoration | `.lua` overwritten by `.bak` |
| **Restoration + deletion** | Restoration + cleanup | `.lua` overwritten, `.bak` deleted |
...
```mermaid
flowchart TD
    A["restore_files(pairs)"] --> B["For each pair"]
    B --> C["shutil.copy2(bak → lua)"]
    C --> D{"Success ?"}
    D -->|Yes| E["Display [OK]"]
    D -->|No| F["Display [FAIL] + error"]
    E --> G["Increment counter"]
    G --> H["Continue"]
    F --> H
    H --> I{"Other pairs ?"}
    I -->|Yes| B
    I -->|No| J["Return num restored"]
```
...
| 📜 | Traceability |  |  |
|--|--|--|--|
| **Name** | *RESTORE_BACKUP.md* | **Version** | 3.1 |
